...
export function mapPostData(post = {}) {
  ...
  // Sanitize and normalize ACF post data obtained from GQL
  // Override initial `post` data with ACF field group for Post
  // Make attribute `post.acf` to get direct access to custom fields
  if (data.post) {
    data.acf = { ...data.post };

    // delete `data.post` to avoid confusion when accessing the object
    delete data.post;
  }
