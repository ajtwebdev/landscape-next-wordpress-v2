...
export function mapPageData(page = {}) {
  ...
  // Sanitize and normalize pageComponent ACF data obtained from GQL
  // Make attribute `page.acf` to get direct access to custom fields
  if (data.pageComponents) {
    data.acf = { flexibleFields: [] };

    data.pageComponents.pageComponents.forEach((element) => {
      if (!('__typename' in element)) {
        return;
      }

      let field = { ...element };

      field.type = element.__typename.replace(/Page_Pagecomponents_/g, '');

      data.acf.flexibleFields.push(field);
    });

    // delete `data.pageComponents` to avoid confusion when accessing the object
    delete data.pageComponents;
  }
